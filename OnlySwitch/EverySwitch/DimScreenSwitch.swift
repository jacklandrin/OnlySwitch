...
class DimScreenSwitch:SwitchProvider {
    ...
    private var autoDimScreenTime = Preferences.shared.autoDimScreenTime * 60 //persistance unit is min, here is second
    ...
    init() {
        NotificationCenter.default.addObserver(forName: .changeDimScreenSetting, object: nil, queue: .main) { [weak self] _ in
            ...
            strongSelf.autoDimScreenTime = Preferences.shared.autoDimScreenTime * 60
        }
    ...
    private func setTimer() {
        timer.sink{ _ in
            ...
            self.timerCounter += 1
            if self.timerCounter == self.autoDimScreenTime {
                self.timerCounter = 0
                self.dimScreen()
