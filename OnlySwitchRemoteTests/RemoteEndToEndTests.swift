...
struct RemoteEndToEndTests {
    ...
    func secureLoopbackDrivesDashboardThroughPairCatalogSubscriptionAndAction() async throws {
        ...
        let pairing = try await RemoteClientSession.pair(
        ...
        pairing.session.startReceiving()
        try await pairing.session.requestCatalog()

        var iterator = messages.makeAsyncIterator()
        guard case let .catalogSnapshot(catalogRevision, controls) = try #require(await iterator.next()) else {
            Issue.record("Expected catalog snapshot")
            return
        }
        #expect(controls == [descriptor])
        try await pairing.session.subscribe([.darkMode])
        guard case let .statusSnapshot(statuses) = try #require(await iterator.next()) else {
            Issue.record("Expected initial status snapshot")
            return
        }
        #expect(statuses == [initialStatus])
        ...
        let store = TestStore(initialState: DashboardFeature.State(
            ...
            statuses: [.darkMode: .init(value: initialStatus, isStale: false)],
            orderedSelectedIDs: [.darkMode],
            connectionState: .authenticated
        )) { DashboardFeature() } withDependencies: {
            $0.uuid = .incrementing
            $0.remoteConnection.send = { try await pairing.session.send($0) }
        }
        store.exhaustivity = .off(showSkippedAssertions: false)

        await store.send(.tileTapped(.darkMode))
        await store.skipReceivedActions()
        var authoritativeStatus: RemoteControlStatus?
        while let message = await iterator.next() {
            if case let .statusChanged(status) = message {
                authoritativeStatus = status
                break
            }
        }
        let receivedStatus = try #require(authoritativeStatus)
        await store.send(.connectionEvent(.status(macID, receivedStatus)))
        #expect(store.state.statuses[.darkMode]?.value == updatedStatus)
        #expect(store.state.statuses[.darkMode]?.isStale == false)
        #expect(await server.operationCount == 1)

        await pairing.session.close()
        await store.send(.connectionEvent(.offline(macID, "Connection closed")))
        #expect(store.state.statuses[.darkMode]?.isStale == true)
